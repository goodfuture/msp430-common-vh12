...
void command_store(INT8U *dat1,INT8U *key)
{
    ...
    switch(s)                                                /*与关键字对应的命令代码存入相应的数组*/
    {
      ...
      case 4:    if(strcmp(LCD_user_pw,dat1)!=0)
                      Rt[0]='3';
                 else Rt[0]='1';             /*dat中的代码与本机存储的pw密码进行匹配*/
      break;

      case 5:    //strcmp(ml_mn,dat1);             /*dat中的代码与本机存储的mn进行匹配*/
      break;

      case 6:    ml_flag=dat1[0];                /*判断并执行flag命令码所对应的操作*/
      break;
 ...
 // if(strcmp(crc,ReData.CRC)!=0)
  ...
  //else
  {
    ...
    else
    {
      ...
      for(i=0,j=1;i<40&&j!=0;i++)                //判断上位机传过来的cn命令代码,与cn代码表进行比对
      j=strcmp(cn[i].cod,ml_cn);
		
      switch(i-1)  //operation();
      {
	case 0:           //设置超时时间与重发次数 1000
          /*i=0;
          cp_analyze();
	  ...
	  //strcpy(parameter.sys_parameter.overtime,interparametre[i].D_intercp);
          ...
          for(k=0;k<5-j;k++)
            LCD_index2[0][k]='0';
          //LCD_indexs(parameter.sys_parameter.overtime,j,LCD_index2[0]+5-j);  //超时时间 5位
	  ...
	  strcpy(parameter.sys_parameter.resendtime,interparametre[i].D_intercp);
          ...
          for(k=0;k<2-j;k++)
            LCD_index1[3][k]='0';
          //LCD_indexs(parameter.sys_parameter.resendtime,j,LCD_index1[3]+2-j);	//重发次数 2位
	  ...
	  //strcpy(parameter.sys_parameter.warntime,interparametre[i].D_intercp);
          ...
          for(k=0;k<5-j;k++)
            LCD_index2[1][k]='0';
	  //LCD_indexs(parameter.sys_parameter.warntime,j,LCD_index2[1]+5-j);  //报警时间 5位
	...
	break;
        case 4:          //提取污染物报警门限值 1021
          ...
          for(i=0;i<8;i++)
          {
            ...
            if(interparametre[j].D_intercp[0]!='\0')
            {
              ...
              //float_to_string(AD_parameter.AD_i[i].L_Alarmlimt.b,ftstring); //float型转换为string
              strcat(CBAG.cbag_dynamic,ftstring);
              strcat(CBAG.cbag_dynamic,comma);
              strcat(CBAG.cbag_dynamic,interparametre[j].D_intercp);
              float_to_string(ADPARA(i).U_Alarmlimt.b,ftstring);
              //float_to_string(AD_parameter.AD_i[i].U_Alarmlimt.b,ftstring); //float型转换为string
	  ...
	  cp_analyze();
	    ...
	    Response(2);
          ...
          //LCD_indexs(parameter.sys_parameter.reporttime,j,LCD_index2[2]+4-j);	//数据上报间隔 2位
          Symbol.LCD_para_set=1;
          e_result[0]='1';
          Response(2);
        break;
	  ...
	  cp_analyze();
          ...
          for(k=0;k<3-j;k++)
            LCD_index2[3][k]  ='0';
            //LCD_indexs(parameter.sys_parameter.rd_interval,j,LCD_index2[3]+3-j);//实时上报间隔 3位
...
float string_to_float(INT8U *tempstr) //reentrant
{
  ...
  for(i=0;*tempstr!='.'&&*tempstr!='\0';tempstr++,i++); //查找小数点位置
...
void Reporthisdata_M(void)
{
  ...
  for(i=0;i<8;i++)
  {
    ...
    //local_mma_D[i].avg +=local_mma_H[i].avg;  //计算小时累计值
    //local_mma_D[i].num++;
    for(j=0;j<3;j++)
    {
      d[j]=ADPARA(i).po_id[j];
    ...
    if((!strcmp(d,"B01")))
    {
      ...
      coutmp =local_mma_M[i].sum/1000; //L/s 单位转换L->m3
...
void Reporthisdata_H(void)
{
  ...
  for(i=0;i<8;i++)
  {
    ...
    if((!strcmp(d,"B01")))
    {
      ...
      strcat(CBAG.cbag_dynamic,d);
      /*环保局要求废水流量实时值单位为L/s,累计值单位为m3*/
      coutmp =local_mma_H[i].sum/1000; //L/s 单位转换L->m3
      local_mma_D[i].sum +=coutmp;
      //coutmp =local_mma_H[i].avg;  //m3/h
      //coutmp =b01h_cou;
      //local_mma_H[i].avg
